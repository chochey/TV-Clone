...
const DEFAULTS = {
  ...
  tiers: { container: true, audio: true, legacy: false, video: false },
  concurrency: 1,
  cpuCores: 2,
  videoCrf: 20,
  niceness: 19,
...
function sanitize(input, current) {
  ...
  if (src.tiers && typeof src.tiers === 'object') {
    ...
    for (const k of ['container', 'audio', 'legacy', 'video']) {
      if (typeof src.tiers[k] === 'boolean') out.tiers[k] = src.tiers[k];
    }
  }

  out.concurrency = 1; // one active file: predictable CPU, IO, and retention budgets
  if (src.cpuCores !== undefined) out.cpuCores = Math.round(clamp(src.cpuCores, 1, require('os').availableParallelism(), current.cpuCores));
  if (src.videoCrf !== undefined) out.videoCrf = Math.round(clamp(src.videoCrf, 18, 24, current.videoCrf));
