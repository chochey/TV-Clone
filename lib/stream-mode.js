...
function computeStreamMode({ ext, codec, audioCodec, pixFmt, audioChannels }) {
  ...
  if (codec === 'h264' && ext === '.mp4' && (!audioCodec || DIRECT_AUDIO.has(audioCodec))) {
    // Multichannel AAC (5.1/7.1) → remux so the audio is downmixed to stereo.
    // Firefox can't decode multichannel AAC and blacks out the whole element.
    // A channel count of 0/undefined means "not probed" — leave as direct.
    if (audioChannels > 2) return 'remux';
    return 'direct';
  }
