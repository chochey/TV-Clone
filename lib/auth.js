...
const VALID_PERMISSIONS = ['canDownload', 'canScan', 'canRestart', 'canLogs', 'canDashboard', 'canOrganizer', 'canNotify'];
...
module.exports = function createAuth({ DATA_DIR, COOKIE_NAME, ADMIN_COOKIE_NAME, saveJSON, cleanupIntervalMs }) {
  ...
  function resolveAdminToken(req) {
    ...
    // Hold the header path to the same shape as the cookie: without this any
    // string registers a new map entry, so a caller could grow adminTokens
    // without bound.
    const aToken = adminMatch ? adminMatch[1] : (/^[a-f0-9]{48}$/.test(headerToken || '') ? headerToken : null);
    ...
    revokeAdminToken(sToken); // one admin token per session, so re-registration can't accumulate
    adminTokens.set(aToken, sToken);
    return true;
  }

  // The admin-token cookie proves "this browser logged in as someone who was
  // offered admin tools" — it is NOT proof of the admin ROLE, because
  // resolveAdminToken re-registers an unrecognised token against whatever
  // session presented it (see above). Authorization must come from the
  // server-side session, so check the role here as well; the token stays as
  // defence in depth.
