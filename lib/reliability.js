...
async function backup(options = {}) {
  ...
  // Retention: keep the newest N daily snapshots. The scheduled backup runs
  // this daily, so without pruning data_backups grows without bound.
  const keep = Math.max(1, parseInt(process.env.TVCLONE_BACKUP_KEEP || '', 10) || 14);
  const pruned = [];
  try {
    const dirs = fs.readdirSync(settings.backupRoot, { withFileTypes: true })
      .filter(e => e.isDirectory() && /^\d{4}-\d{2}-\d{2}$/.test(e.name))
      .map(e => e.name)
      .sort()              // ISO dates sort chronologically
      .reverse();          // newest first
    for (const name of dirs.slice(keep)) {
      try { fs.rmSync(path.join(settings.backupRoot, name), { recursive: true, force: true }); pruned.push(name); } catch {}
    }
  } catch {}

  return { ok: true, target, copied, kept: keep, pruned };
}
