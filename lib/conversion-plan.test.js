...
test('subtitle risk is only evaluated when the container is changing', () => {
  // Already-MP4 files (audioOnly tier) keep their container untouched, so an
  // embedded image subtitle there is not this feature's problem.
  ...
  assert.strictEqual(untouched.hasImageSubs, false);
