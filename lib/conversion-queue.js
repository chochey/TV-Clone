...
module.exports = function createConversionQueue({
  ...
  async function loop() {
    ...
    try {
      while (status === 'running' && cursor < queue.length) {
        ...
        current = filePath;
        const result = await worker.convertContainerOnly(filePath);
  ...
  function pause() {
    ...
    status = 'paused';
    log('paused — the file in flight will finish first');
    return { ok: true };
  }
  ...
  function stop() {
    ...
    if (wasPaused) {                          // no loop to notice the flag
      status = 'idle';
      if (converted > 0) { try { onBatchComplete(); } catch {} }
    }
  ...
  return { start, pause, resume, stop, snapshot, get status() { return status; } };
};
