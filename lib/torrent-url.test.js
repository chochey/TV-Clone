...
test('magnet URIs are always addable', () => {
  assert.strictEqual(classifyTorrentUrl('magnet:?xt=urn:btih:abc123'), 'magnet');
  assert.strictEqual(classifyTorrentUrl('MAGNET:?xt=urn:btih:ABC'), 'magnet');
});
...
test('a .php path that renders a page is refused, but a query on a real path is not', () => {
  assert.strictEqual(classifyTorrentUrl('https://tracker/get.php'), 'page');
...
test('leading/trailing whitespace is tolerated', () => {
  assert.strictEqual(classifyTorrentUrl('  magnet:?xt=urn:btih:z  '), 'magnet');
