...
test('concurrency stays at one to bound resource usage', () => {
  assert.strictEqual(sanitize({ concurrency: 99 }, DEFAULTS).concurrency, 1);
...
test('non-numeric values fall back to the current setting rather than becoming NaN', () => {
  ...
  assert.strictEqual(out.concurrency, 1);
...
test('CPU allocation is bounded by the available machine cores and quality stays in a useful range',()=>{
 const out=sanitize({cpuCores:999,videoCrf:99},DEFAULTS);
 assert.strictEqual(out.cpuCores,require('os').availableParallelism());assert.strictEqual(out.videoCrf,24);
 assert.strictEqual(sanitize({cpuCores:0,videoCrf:0},DEFAULTS).cpuCores,1);
});
