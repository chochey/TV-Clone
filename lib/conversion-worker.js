...
module.exports = function createConversionWorker({
  ...
  // Delete retained originals past their keep-days. Two guards, both
  // load-bearing:
  //
  //   * expired only — an original inside its window is the undo button, and
  //     deleting it early silently removes the ability to recover.
  //   * restorable only — `restorable` means the converted replacement is
  //     still sitting where we left it. If it is NOT, then this retained
  //     original is the ONLY remaining copy of that content, and deleting it
  //     destroys the file outright. That is the difference between reclaiming
  //     space and losing media, so it is checked per file at delete time
  //     rather than trusted from a stale listing.
  function cleanupExpiredOriginals(roots, keepDays, { dryRun = false } = {}) {
    const all = listRetainedOriginals(roots, keepDays);
    const deletable = all.filter((o) => o.expired && o.restorable);
    const keptBecauseOnlyCopy = all.filter((o) => o.expired && !o.restorable);
    const deleted = [];
    let bytes = 0;
  ...
  return {
    ...
    cleanupExpiredOriginals, killActive,
  };
};
