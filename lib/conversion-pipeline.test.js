...
test('manual cleanup previews recent originals and deletes only the selected original',t=>{
 const f=fixture(t),a=retainedFixture(f,'one'),b=retainedFixture(f,'two');
 assert.equal(f.worker.cleanupExpiredOriginals([f.dir],14,{dryRun:true}).candidates,0);
 const preview=f.worker.cleanupExpiredOriginals([f.dir],14,{dryRun:true,includeRecent:true,retainedPaths:[a]});
 assert.equal(preview.candidates,1);assert.equal(fs.existsSync(a),true);
 const result=f.worker.cleanupExpiredOriginals([f.dir],14,{includeRecent:true,retainedPaths:preview.retainedPaths});
...
test('delete all protects missing replacements and files added after the preview',t=>{
 const f=fixture(t),a=retainedFixture(f,'one'),only=retainedFixture(f,'only',false);
 const preview=f.worker.cleanupExpiredOriginals([f.dir],14,{dryRun:true,includeRecent:true});
...
test('manual deletion cannot select paths outside the enumerated library originals',t=>{
 const f=fixture(t),a=retainedFixture(f,'one');
 const result=f.worker.cleanupExpiredOriginals([f.dir],14,{includeRecent:true,retainedPaths:[path.join(f.dir,'one.mp4'),'/tmp/unrelated']});
