...
test('a true re-encode still emits MPEG-TS', () => {
  // The AVCC→Annex-B corruption only affects the copy path; a re-encode
  // produces a fresh conformant stream, so TS stays for transcode.
  const { segmentType } = build({ mode: 'transcode', videoCodec: 'h264' });
  assert.strictEqual(segmentType, 'ts');
});
