...
const { buildFfmpegArgs, DELIVERY_MODES } = require('./ffmpeg-args');
const fs = require('fs');
...
test('HLS output options are always appended last', () => {
  for (const over of [{}, { mode: 'transcode' }, { mode: 'transcode', vaapiAvailable: false }]) {
    const { args, segmentType } = build(over);
    ...
    assert.strictEqual(pairAfter(args, '-hls_segment_filename'), `/tmp/sess/seg_%04d.${ext}`);
  }
});
...
test('passthrough does not leak into non-HEVC sources', () => {
  ...
  assert.strictEqual(h264.segmentType, 'fmp4');
});
...
test('delivery reports the path actually taken', () => {
  // Drives the Now Watching badge — a wrong label here is worse than none,
  // because it would misdirect the next person debugging a slow stream.
  assert.strictEqual(build({ mode: 'remux', videoCodec: 'h264' }).delivery, 'remux');
  assert.strictEqual(build({ ...hevc, hevcPassthrough: true }).delivery, 'passthrough');
  assert.strictEqual(build(hevc).delivery, 'transcode-gpu');
  assert.strictEqual(build({ ...hevc, vaapiAvailable: false }).delivery, 'transcode-cpu');
  // A source promoted out of copying because it exceeds the cap is a
  // transcode, and must say so rather than still claiming remux.
  assert.strictEqual(build({ mode: 'remux', videoCodec: 'h264', srcHeight: 2160 }).delivery, 'transcode-gpu');
});

test('every delivery mode the builder can emit has a dashboard badge', () => {
  // The Now Watching badge is a lookup table in Dashboard.svelte. If a new mode
  // is added to the builder and not to that table, the UI renders a raw slug.
  // Reading the source is crude but catches the drift, which a unit test on
  // either side alone cannot.
  const dash = fs.readFileSync(
    path.join(__dirname, '..', 'v2', 'frontend', 'src', 'routes', 'Dashboard.svelte'), 'utf8');
  const table = dash.slice(dash.indexOf('const DELIVERY'), dash.indexOf('};', dash.indexOf('const DELIVERY')));
  for (const mode of DELIVERY_MODES) {
    assert.ok(table.includes(`'${mode}'`) || table.includes(`${mode}:`),
      `Dashboard.svelte has no badge for delivery mode "${mode}"`);
  }
  // 'direct' has no ffmpeg session at all, so the builder never emits it, but
  // the endpoint synthesises it and the badge table must still cover it.
  assert.ok(table.includes('direct:'), 'Dashboard.svelte has no badge for "direct"');
});

test('H.264 remux emits fMP4 tagged avc1, not MPEG-TS', () => {
  // Copying H.264 from MP4 into MPEG-TS needs an AVCC→Annex-B rewrite that
  // corrupts some encodes (a YTS "The Last House" release decoded to
  // "non-existing PPS"/"no frame!" and failed playback). fMP4 keeps AVCC and
  // skips the rewrite, so every copy path is fMP4 now.
  const { args, segmentType } = build({ mode: 'remux', videoCodec: 'h264' });
  assert.strictEqual(segmentType, 'fmp4');
  assert.strictEqual(pairAfter(args, '-hls_segment_type'), 'fmp4');
  assert.strictEqual(pairAfter(args, '-tag:v'), 'avc1', 'H.264 fMP4 sample entry must be avc1');
  assert.strictEqual(pairAfter(args, '-hls_fmp4_init_filename'), 'init.mp4');
  assert.ok(pairAfter(args, '-hls_segment_filename').endsWith('seg_%04d.m4s'));
});

test('a true re-encode also emits fMP4 tagged avc1', () => {
  // MPEG-TS transcode forced hls.js to transmux Annex-B → AVCC into MSE.
  // Firefox on Linux flashes garbage frames on those appends — including
  // while paused, and with hardware decode off. fMP4 is already AVCC, so
  // MSE appends the encoder's own stream. Same container as remux.
  const { args, segmentType } = build({ mode: 'transcode', videoCodec: 'h264' });
  assert.strictEqual(segmentType, 'fmp4');
  assert.strictEqual(pairAfter(args, '-hls_segment_type'), 'fmp4');
  assert.strictEqual(pairAfter(args, '-tag:v'), 'avc1');
  assert.ok(pairAfter(args, '-hls_segment_filename').endsWith('seg_%04d.m4s'));
});


test('subtitles stay out of HLS for every encoding path, including default audio selection', () => {
  for (const mode of ['remux', 'transcode']) {
    for (const vaapiAvailable of [false, true]) {
      for (const hevcPassthrough of [false, true]) {
        const { args } = buildFfmpegArgs({ filePath: '/fixture.mkv', sessionDir: '/tmp/session',
          seekTime: 1049, mode, preset: { maxH: 1080, crf: 23, vaapiQp: 22, maxrate: '4M', bufsize: '8M' },
          pixFmt: 'yuv420p10le', videoCodec: 'hevc', audioCodec: 'ac3', audioChannels: 6,
          srcHeight: 1080, vaapiAvailable, vaapiCanDecode: true, hevcPassthrough, threads: 2, segDuration: 4 });
        assert.ok(args.includes('-sn'), 'embedded cues must not shift the audio/video timeline');
        assert.ok(args.indexOf('-sn') > args.indexOf('-i'), 'disable subtitles on the output');
      }
    }
  }
});
