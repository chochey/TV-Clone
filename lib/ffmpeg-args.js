...
function buildFfmpegArgs(o) {
  ...
  // Every HLS session is fragmented MP4. Copy paths need it so H.264 stays
  // AVCC (MPEG-TS rewrites AVCC→Annex-B and corrupted a YTS "The Last House"
  // encode into a fatal "no frame!" media error). Re-encode paths need it
  // too: MPEG-TS forced hls.js to transmux into MSE, and Firefox on Linux
  // flashes garbage frames on those appends — including while paused, and
  // with hardware decode off. fMP4 is already AVCC, so MSE gets the encoder's
  // own stream. HEVC passthrough was fMP4 already (hls.js cannot transmux it).
  const segmentType = 'fmp4';
  ...
  // The player fetches subtitles separately. Auto-muxed subtitle cues before
  // a seek can shift fMP4 timestamps several seconds forward (Burn Notice
  // S04E08 at 1049s: video starts at 5.352s instead of 0.021s), leaving
  // Firefox buffering at zero. Keep them out of the audio/video playlist.
  args.push('-sn');
  ...
  if (copyingVideo) {
    ...
    if (is10bit || !vaapiCanDecode || needsDownscale) {
      ...
      args.push(
        ...
        '-c:v', 'h264_vaapi', '-profile:v', 'main', '-tag:v', 'avc1',
        '-qp', String(preset.vaapiQp), '-maxrate', preset.maxrate, '-bufsize', preset.bufsize,
      );
    } else {
      ...
      args.push('-c:v', 'h264_vaapi', '-profile:v', 'main', '-tag:v', 'avc1',
        '-qp', String(preset.vaapiQp), '-maxrate', preset.maxrate, '-bufsize', preset.bufsize);
    }
    ...
    args.push(
      ...
      '-c:v', 'libx264', '-preset', 'ultrafast', '-profile:v', 'main', '-tag:v', 'avc1',
