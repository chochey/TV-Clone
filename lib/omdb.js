...
module.exports = function createOmdb({
  ...
  // Tracked incrementally — cacheVersion is read on every /api/library
  // request, so it must not scan the whole cache each time.
  let omdbMaxFetchedAt = 0;
  for (const value of Object.values(omdbCache)) {
    if (value && value._fetchedAt > omdbMaxFetchedAt) omdbMaxFetchedAt = value._fetchedAt;
  }
  ...
  function markOmdbDirty() {
    omdbCacheDirty = true;
    omdbCacheVersion++;
    // Every write path stamps _fetchedAt: Date.now() just before this call.
    omdbMaxFetchedAt = Date.now();
  }
  ...
  // Letters-only, lowercase: "R I P D" and "R.I.P.D." both -> "ripd".
  function alphaKey(s) {
    return String(s || '').toLowerCase().replace(/[^a-z0-9]/g, '');
  }

  // OMDb's t= match is fuzzy: a query for "R I P D" can return the unrelated
  // short "D R I P". Flag results whose title shares no alpha-key resemblance
  // with the query so the variation path runs instead of accepting garbage.
  function isTitleMismatch(query, data) {
    if (!data || data.Response === 'False' || !data.Title) return false;
    const q = alphaKey(query), t = alphaKey(data.Title);
    if (!q || !t) return false;
  ...
  // Dotted-acronym variants. Titles parsed from filenames lose their dots:
  // "R.I.P.D. 2: Rise of the Damned" becomes "R I P D 2 Rise of the Damned",
  // which OMDb can't match. Regenerate dotted/collapsed/unglued-digit forms
  // so one of the exact queries lands. Mirrors the organizer's logic.
  function acronymVariations(title) {
    const out = [];
    const base = title.replace(/\s+/g, ' ').trim().replace(/\.+$/, '').trim();
    const add = (value) => {
      const v = (value || '').replace(/\s+/g, ' ').trim();
      if (v && v.toLowerCase() !== title.toLowerCase() && !out.includes(v)) out.push(v);
    };
    // Runs of 2+ single letters: "S W A T" -> "S.W.A.T." and "SWAT"
    const run = /\b([A-Za-z](?:\s+[A-Za-z]\b)+)/;
    if (run.test(base)) {
      add(base.replace(new RegExp(run, 'g'), m => m.split(/\s+/).join('.') + '.'));
      add(base.replace(new RegExp(run, 'g'), m => m.split(/\s+/).join('')));
    }
    // Digit glued onto a dotted acronym: "R.I.P.D.2. Rise" -> "R.I.P.D. 2 Rise"
    let unglued = base.replace(/((?:[A-Za-z]\.){2,})\s*(\d)/g, '$1 $2');
    unglued = unglued.replace(/(\d)\.(?=\s|$)/g, '$1');
    add(unglued);
    // Dotted acronym -> plain word: "S.W.A.T. Under Siege" -> "SWAT Under Siege"
    add(unglued.replace(/\b((?:[A-Za-z]\.){2,})/g, m => m.replace(/\./g, '')));
    return out;
  }
  ...
  function lookupVariations(title) {
    ...
    for (const v of acronymVariations(title)) add(v);
  ...
  async function fetchOmdbData(title, year, type, options = {}) {
    ...
    try {
      ...
      if (data.Response === 'False' || isWeakOmdbData(data) || isTitleMismatch(title, data)) {
        ...
        for (const variant of variations) {
          try {
            ...
            if (vData.Response !== 'False' && !isWeakOmdbData(vData) && !isTitleMismatch(variant, vData)) {
  ...
  return {
    ...
    lookupVariations, isTitleMismatch, isWeakOmdbData,
    get cacheSize() { return Object.keys(omdbCache).length; },
    get cacheVersion() {
      return `${Object.keys(omdbCache).length}-${omdbMaxFetchedAt}-${omdbCacheVersion}`;
    },
  };
};
